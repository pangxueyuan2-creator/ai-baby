...
`/forget ID` 让指定有效事实以及关联 / 匹配经历不再参与检索，并清理关联及匹配的追问。为避免旧副本再次唤起内容，同时清空近期对话、缓存回复正文、待确认候选和全部旧日记；剩余有效经历可再次生成日记。最近保留的请求 ID 会标记为撤销，旧请求重试被拒绝，不能重新学习已忘内容；仍只保留最近 256 条请求身份，超出窗口不保证去重。人格和已经达到的阶段不倒退，记忆计数重新计算。**失效记录、请求摘要和已有备份仍在磁盘；这是逻辑遗忘，不是安全擦除。** 重新以新请求明确教学，仍可再次保存同一内容。
