...
外部地址要求 HTTPS；回环 `localhost / 127.0.0.1 / ::1` 允许 HTTP，便于本地兼容服务。本地无鉴权服务可以不填 key（不发送 Authorization），或填写自己的 dummy key。回环请求会绕过系统代理，避免本地模型数据意外经过代理。远程服务仍要求 key。URL 不允许内嵌凭据或查询串，禁止跟随 HTTP 重定向。已有本地 OpenAI-compatible 服务可填自己的地址和模型 ID，仍需两个显式开关；项目不会下载模型或启动模型服务。

总等待期限可设置 1–120 秒。默认不重试；可设置最多 2 次重试，仅针对明确 HTTP 429 拒绝，退避最多 2 秒。超时、断连、5xx、认证失败等不自动重试。Ctrl+C 可迅速结束调用者等待，并中断已登记 socket 的响应头 / 响应体读取，包括缓慢滴流的响应。已经送达服务的请求不能撤回；DNS 解析、连接建立或 TLS 握手仍可能等待系统或 socket 超时，不能承诺线程瞬时结束。取消后结果不会写入数据库，也不会再安排重试。在旧线程结束前，同一 provider 的并发或后续请求会提示 busy 并离线回复，避免线程不断堆积。暂不支持 streaming。
